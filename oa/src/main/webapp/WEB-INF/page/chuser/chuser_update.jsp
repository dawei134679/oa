...
$(function(){
		 ...
		 success:function(res){
			 if(res.code!=200){
				 ...
				 $(res.data).each(function(index, element){
						 ...
						 if(element.id == "${data.warehouseId }" ){
							 $("#warehouseName").append("<option selected = 'selected' value="+element.id+">"+element.name+"</option>");
						}else {
							 $("#warehouseName").append("<option value="+element.id+">"+element.name+"</option>");
						}  
				 });
	...
	$("#subBtn").click(function(){
		 ...
		 $.ax({
			url:'${ctx}/admin/SWhUser/updateSWhUser',
			data:{'id':id,'warehouseId':warehouseId,'uid':uid},
			success:function(res){
					if(res.code!=200){
